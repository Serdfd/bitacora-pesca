...
// suncalc para amanecer/atardecer
// npm install suncalc @types/suncalc
let SunCalc: any = null
try { SunCalc = require('suncalc') } catch { /* sin suncalc, usamos fallback */ }
...
function getSolTiempos(fecha: Date): { amanecer: Date; atardecer: Date } {
  ...
  // Fallback para Puerto Escondido (aprox 5:50am - 6:10pm)
...
export default function Hoy() {
  ...
  const [mareasDia, setMareasDia] = useState<ReturnType<typeof getMareasDia>>([])
  ...
  const { amanecer, atardecer } = getSolTiempos(hoy)
  ...
  useEffect(() => {
    setCargando(true)
    getDatosClimaticos(hoy).then(d => {
      setClima(d)
      setSenales(getSenalesAgua(d, lunar.fase))
      setVentanas(getVentanasPesca(hoy, amanecer, atardecer))
      setMareasDia(getMareasDia(hoy))
      setCargando(false)
    })
  }, [])

  // Todas las especies ordenadas por calificación
  ...
  return (
    <div className="space-y-6">
      ...
      {cargando ? (
        <Card>
          <div className="flex items-center justify-center py-8 text-ocean-400">
            <span className="animate-pulse">⏳ Cargando condiciones meteorológicas...</span>
          </div>
        </Card>
      ) : clima && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              <SectionTitle icono="🌤️" titulo="Clima Actual" />
              <span className={`text-xs px-2 py-1 rounded-full ${
                clima.fuenteDatos === 'api'
                  ? 'bg-green-500/20 text-green-400 border border-green-500/30'
                  : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'
              }`}>
                {clima.fuenteDatos === 'api' ? `🛰 API · ${clima.ultimaActualizacion}` : '📊 Estimado estacional'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className="bg-ocean-800/40 rounded-xl p-2.5 text-center">
                  ...
                  <p className="text-white text-sm font-semibold leading-tight">{d.valor}<span className="text-ocean-400 text-xs">{d.sub}</span></p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {especiesCalificadas.map(({ especie, cal }) => {
              ...
              return (
                <div
                  key={especie.id}
                  className={`flex items-center gap-3 rounded-xl p-3 border ${s.bg} border-current/20`}
                >
                  <div className="flex-1 min-w-0">
