...
const EMPTY_ZONA: FormZona = { nombre: '', descripcion: '', estado: 'recomendado' }
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              onChange={e => setF('recomendaciones', e.target.value)}
              rows={3}
              placeholder="Técnicas que funcionan, horarios, precauciones..."
              className="bg-ocean-800/60 border border-white/10 rounded-xl px-3 py-2 text-white text-sm placeholder-ocean-600 focus:outline-none focus:border-amber-500/50 resize-none transition-colors"
            />
          </div>
        </div>
  ...
  return (
    <div className="flex items-start gap-3 bg-ocean-800/40 rounded-xl p-3 border border-white/10">
      ...
      <div className="flex-1 min-w-0">
        ...
        {spot.tipo && (
          <p className="text-ocean-400 text-xs mt-0.5">{spot.tipo}</p>
        )}
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-ocean-700/60 border border-white/10 text-ocean-300 hover:text-white hover:bg-ocean-600/60 transition-all"
          >
            + Nuevo Spot
          </button>
...
export default function Zonas() {
  ...
  return (
    <div className="space-y-5">
      ...
      {regionesFiltradas.length === 0 ? (
        <Card className="p-16 text-center">
          ...
          <p className="text-ocean-400 text-sm">
            {regiones.length === 0
              ? 'Crea tu primera zona de pesca para comenzar.'
      ...
      {modalNuevaZona && (
        <ModalNuevaZona
          onClose={() => setModalNuevaZona(false)}
          onGuardar={guardarNuevaZona}
        />
      )}
      {zonaParaSpot && (
        <ModalNuevoSpot
